...
const BuyCoinsComponent = ({ onCoinsUpdate }) => {
  ...
  const fetchPaymentSheetParams = async (value) => {
    ...
    const response = await fetch(`http://192.168.1.25/api/createPaymentIntent?price=${value}&coins=${coinsData.find(coin => coin.price === value)?.amount}`, {
      method: 'GET',
      headers: {
  ...
  async function handleBuyNow(value) {
    ...
    if (paymentResponse.error) {
      ...
      const coinAmount = coinsData.find(coin => coin.price === value)?.amount || 0;
      onCoinsUpdate(coinAmount); // Update the coin state in the parent component
    }
